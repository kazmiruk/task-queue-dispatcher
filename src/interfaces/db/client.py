...
class DBConnection(object):
    ...
    def _get_collection_name(self):
        ...
        collection
        """
        return "{schema}.{channel_name}".format(schema=self._schema, channel_name=self._channel_name())

    def _channel_name(self):
        """ generates channel name from queue name and chunk
        """
        return "{queue_name}_{chunk}".format(queue_name=OPTIONS.queue_name, chunk=OPTIONS.chunk)
    ...
    def delete_row(self, object_id):
        ...
        has select postfix
        """
        result = self._ipoll.run(self._get_query("DELETE FROM {data_path} WHERE id = %s; SELECT 1 as status"),
                                 [object_id])

        try:
            status = result.get(timeout=gevent.Timeout(settings.WAIT_DB_RESPONSE))[0]['status']
        except gevent.Timeout, e:
            status = False

        logging.debug("Row from {collection_name} with id {id} deleted with status {status}".format(
            collection_name=self._get_collection_name(), id=object_id, status=status))
    ...
    def select_all(self):
        ...
        demon
        """
        result = self._ipoll.run(self._get_query("SELECT id, payload, sked_time FROM {data_path}"), [])

        tasks = result.get()

        logging.debug("There are {count} rows save in db".format(count=len(tasks)))
    ...
    def listen(self, rq):
        ...
        JobWrapper.spawn(self._ipoll.listen_on, result_queue=rq, channel_name=self._channel_name(),
